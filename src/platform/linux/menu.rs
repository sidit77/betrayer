...
impl<T: Clone + Send + 'static> DBusMenu<T> {
    pub async fn update_menu(
        ...
        if !updated.is_empty() || !removed.is_empty() {
            log::trace!(
                "Sending property update signal (Updated: {updated:?}, Removed: {removed:?}"
            );
        ...
        let entry = match item {
            ...
            MenuItem::CheckButton {
                ...
                properties: HashMap::from([
                    ...
                    (
                        String::from("toggle-type"),
                        OwnedValue::from(Str::from_static("checkmark")),
                    ),
                    (
                        String::from("toggle-state"),
                        OwnedValue::from(if checked { 1i32 } else { 0i32 }),
                    ),
                ]),
                children: vec![],
                signal: Some(signal),
...
impl<T: Clone + Send + 'static> DBusMenu<T> {
    ...
    fn get_group_properties(
        ...
        entries
            ...
            .enumerate()
            .filter_map(|(i, e)| {
                (ids.is_empty() || ids.contains(&(i as i32)))
    ...
    fn get_property(&self, id: i32, name: &str) -> OwnedValue {
        ...
        self.entries
            .lock()
            .get(id as usize)
            .and_then(|e| e.properties.get(name).cloned())
            .unwrap()
    }
